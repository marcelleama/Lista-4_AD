...
#Vamos tratar as bases para ficar mais fácil de operá-las:
...
#adicionando uma variável como o enunciado pede:

docentes_pe_selecao <- docentes_pe %>% mutate(FX_IDADE = ifelse(NU_IDADE >=18, NU_IDADE <= 70, "de 18 anos a 70 anos"))
head(docentes_pe_selecao)
summary(factor(docentes_pe_selecao$FX_IDADE))

# criamos uma nova base agrupando as duas variáveis de interesse:
# criamos subtotais combinando as funcoes group by e summarise:
...
#incluimos uma nova variavel na base com a condicao da questao: entre 01 e 25 anos

matricula_pe_selecao <- matricula_pe %>% mutate(FX_IDADE = ifelse(NU_IDADE >=1, NU_IDADE <=25, "de 01 a 25 anos"))
...
#moda
moda <- c(sample(1:10, 100, replace = TRUE))
table(moda)[which.max(table(moda))]
1 
14
...
# a o valor de r indica uma correlação fraca entre as variáveis.
